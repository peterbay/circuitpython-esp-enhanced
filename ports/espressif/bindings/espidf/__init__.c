...
static mp_obj_t espidf_pbkdf2(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    int ret = mbedtls_pkcs5_pbkdf2_hmac_ext(MBEDTLS_MD_SHA256,
        password.buf, password.len, salt.buf, salt.len,
        (unsigned int)iterations, (uint32_t)key_length, (unsigned char *)vstr.buf);
    if (ret != 0) {
        vstr_clear(&vstr);
...
static void espidf_gcm_setkey(mbedtls_gcm_context *ctx, mp_obj_t key_in) {
    ...
    // Without this the failure surfaces as an opaque setkey error.
    if (key.len == 24) {
        mp_raise_ValueError(MP_ERROR_TEXT("AES-192 is not supported by this chip"));
    }
    #endif
    mbedtls_gcm_init(ctx);
    if (mbedtls_gcm_setkey(ctx, MBEDTLS_CIPHER_ID_AES, key.buf, key.len * 8) != 0) {
...
static mp_obj_t espidf_aes_gcm_encrypt(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    espidf_gcm_setkey(&ctx, args[ARG_key].u_obj);

    vstr_t vstr;
    vstr_init_len(&vstr, data.len + ESPIDF_GCM_TAG_LEN);
    unsigned char *out = (unsigned char *)vstr.buf;
    int ret = mbedtls_gcm_crypt_and_tag(&ctx, MBEDTLS_GCM_ENCRYPT, data.len,
        nonce.buf, nonce.len, aad.buf, aad.len, data.buf, out,
        ESPIDF_GCM_TAG_LEN, out + data.len);
    mbedtls_gcm_free(&ctx);
    if (ret != 0) {
        vstr_clear(&vstr);
...
static mp_obj_t espidf_aes_gcm_decrypt(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    int ret = mbedtls_gcm_auth_decrypt(&ctx, body, nonce.buf, nonce.len,
        aad.buf, aad.len, (const unsigned char *)data.buf + body, ESPIDF_GCM_TAG_LEN,
        data.buf, (unsigned char *)vstr.buf);
    mbedtls_gcm_free(&ctx);
    if (ret != 0) {
