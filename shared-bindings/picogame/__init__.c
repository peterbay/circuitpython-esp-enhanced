...
// floor((a * dd) / 65536) without a 64-bit multiply, for a <= 2^16 and dd <= 2^24: splitting dd
// into halves keeps both partial products inside 32 bits (the larger is 2^16 * (2^16 - 1)). Exact,
// not an approximation. Targets with a widening multiply (ARMv7-M SMULL, Xtensa MULUH, RV32M
// MULHU) would do the 64-bit form in about as many instructions; this is the portable shape that
// is also good on ARMv6-M, which has none. Used by the noise kernel and the raycaster below.
...
// Everything below is 32-bit. The three places this kernel used int64 all have exact 32-bit
// forms, and "exact" here means proved, not measured: smooth16 was checked over its entire
// 65536-value domain, lerp16 over all 8.59e9 (delta, u) pairs it can be handed, and the whole
// sampler over 8.6M coordinate/seed combinations including random ones across the full int32
// lattice - zero mismatches against the int64 originals. That matters because an __aeabi_lmul
// costs ~25 cycles on the M0+ and an octave is 680.

// The avalanche half of the hash. Split out because the four corner hashes of a cell differ
// only by constants in their input: the coordinate terms are linear in uint32, so
// h(xi+1, yi) == h(xi, yi) + 374761393. Computing the coordinate part once for the cell turns
// 16 multiplies into 7, bit for bit the same four values.
...
static inline uint32_t pg_lerp16(uint32_t a, uint32_t b, uint32_t u) {
    // |b - a| and u are both under 2^16, so the product needs 32 bits but not 33 - except that
    // it is signed. Split on the sign and keep it unsigned: for a negative delta the original
    // arithmetic shift floors, and floor(-m / 65536) == -((m + 65535) >> 16).
...
// buffer (perpendicular distance, 16.16). Every arithmetic step is 32-bit - the per-column setup
// used to reciprocate and scale in int64; see pg_recip32 below and pg_mulshr16 further up for the
// exact 32-bit forms. Mirrors the Python float fallback closely.
...
// floor(2^32 / v). CALLER MUST GUARANTEE v >= 1: dividing by zero is a trap on some hosts, not a
// defined 0. No target CircuitPython builds for has a 64-bit divide instruction, so the obvious
// (1ull << 32) / v is a software routine everywhere; this is one 32-bit divide instead. Identity:
// 2^32/v == floor((2^32 - v)/v) + 1, and 2^32 - v is -v reduced modulo 2^32. Bit-exact, not an
// approximation. The cast pins that reduction to 32 bits rather than to whatever `unsigned` is.
...
static mp_obj_t picogame_raycast(size_t n_args, const mp_obj_t *args) {
    ...
    // Clamped because the DDA below bounds-checks the cell with unsigned compares, which read a
    // negative extent as a huge one. Once per call, not once per step.
    ...
    for (int c = 0; c < ncols; c++) {
        ...
        // deltaDist = |1/rayDir| in 16.16 = (1<<32)/|rayDir_q16|, clamped to DD_CAP. 2^32/ax exceeds
        // DD_CAP exactly when ax < 256, so the cap doubles as the ax == 0 guard and the surviving
        // divide always lands in [1<<16, 1<<24]. (Per-column setup, not per-step.)
        ...
        int side = 0;
        int cell = 1;
        // Walk a linear offset alongside the cell coords. The bounds test still needs mapx/mapy
        // separately, but the fetch no longer redoes mapy * mw + mapx on every one of up to 64
        // steps - the row stride is one multiply per column instead.
        int off = off0;
        ...
        for (int i = 0; i < 64; i++) {             // DDA - pure 32-bit
            ...
            // Unsigned compares: a negative coordinate wraps to a huge unsigned and fails the same
            // branch, so two tests replace four. This runs up to 64 times per column.
