...
static mp_uint_t PLACE_IN_WARM_CODE(get_fmt_num)(const char **p) {
    ...
    // CIRCUITPY-CHANGE: the count is multiplied by the item size in three
    // places, and that product overflowed past both size checks --
    // calcsize("536870912q") came out 0 because 8 * 536870912 wraps. Bound it
    // well below where that can happen; CPython's own limit is far lower still.
