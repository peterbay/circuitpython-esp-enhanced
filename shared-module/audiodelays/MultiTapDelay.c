...
void common_hal_audiodelays_multi_tap_delay_set_delay_ms(audiodelays_multi_tap_delay_obj_t *self, mp_obj_t delay_ms) {
    ...
    // CIRCUITPY-CHANGE: same as Echo -- raising the length to the audio buffer
    // size could put it back above the maximum, and the memset below then
    // cleared an unsigned underflow's worth of memory.
